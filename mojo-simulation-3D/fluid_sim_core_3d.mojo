...
fn key_from_hash(hash_val: Int, table_size: Int) -> Int:
    return hash_val % table_size


fn get_offsets_3d() -> List[SIMD[DType.int32, 4]]:
    var offsets = List[SIMD[DType.int32, 4]]()
    for dz in range(-1, 2):
        for dy in range(-1, 2):
            for dx in range(-1, 2):
                offsets.append(SIMD[DType.int32, 4](Int32(dx), Int32(dy), Int32(dz)))
    return offsets^
...
struct FluidSim3D:
    ...
    fn _build_spatial_hash(mut self):
        # Spatial hash not used - using brute force like original fluid_sim_core.mojo
        pass
    ...
    fn _update_densities(mut self):
        ...
        for i in range(self.particle_count):
            ...
            # Simple brute force neighbor search (no spatial hashing for simplicity, like original)
